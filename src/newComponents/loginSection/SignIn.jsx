...
//       // Show success toast and navigate after close
...
const SignIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:4000/login/login", // adjust this URL if needed
        { email, password, role },
        { withCredentials: true }
      );
      ...
      // Store token, role, user id, and companyId (if employee) in localStorage
      localStorage.setItem("token", token);
      localStorage.setItem("role", user.role);
      ...
      if (user.role === "employee" && user.companyId) {
        ...
        localStorage.removeItem("companyId"); // clear if exists from previous sessions
      }
